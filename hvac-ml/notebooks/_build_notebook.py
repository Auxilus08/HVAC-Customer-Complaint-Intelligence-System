...
def main() -> None:
    ...
    cells.append(code(
        ...
        "size_before = embedder.cache_size\n"
        "embedder.encode_batch(texts[:50])\n"
        "size_after = embedder.cache_size\n"
        "print(f'Cache size unchanged: {size_before} → {size_after} '\n"
        "      f'(expected equal — pure cache hits, no new entries)')\n"
        "\n"
    ...
    cells.append(code(
        ...
        "    print('GOOGLE_API_KEY not set — using deterministic stub labels')\n"
