...
POST /complaints/upload  →  PII strip  →  DB write  →  Redis queue
                                              ...
                                              label_job   (Claude API, Jaccard-gated)
...
```bash
cp .env.example .env
# Fill in ANTHROPIC_API_KEY and RAW_TEXT_ENCRYPTION_KEY
...
- **Embedding cache**: SHA-256 text hash → Redis TTL prevents recomputing identical texts.
